...
async function fetchLeaderboard() {
  try {
    ...
    // Try official leaderboard endpoint
    const response = await axios.get(
      `${POLYMARKET_LEADERBOARD}?period=daily&limit=100`,
      { timeout: 8000 }
    );

    if (!response.data || !response.data.data) {
      console.log('⚠️ Leaderboard endpoint unavailable, using fallback...');
      return [];
    }
    ...
    console.log(`✅ Fetched leaderboard: ${leaderboard.length} users`);
    return leaderboard;
  } catch (error) {
    console.error('Error fetching leaderboard:', error.message);
    
    // Fallback: Simulate leaderboard data from active markets
    return await simulateLeaderboardFromMarkets();
  }
}

// ===== FALLBACK: SIMULATE LEADERBOARD FROM ACTIVE MARKETS =====
async function simulateLeaderboardFromMarkets() {
  try {
    console.log('🔍 Building leaderboard from active market trades...');
    ...
    const leaderboard = Object.values(walletStats)
      .filter(user => user.totalTrades >= 3) // Min 3 trades
      .map(user => ({
    ...
    botState.leaderboardData = leaderboard;
    console.log(`✅ Built leaderboard: ${leaderboard.length} traders`);
    return leaderboard;
  } catch (error) {
...
async function discoverWallets() {
  try {
    ...
    // Filter for users with success rate >= minSuccessRate
    for (const user of leaderboard) {
      const winRate = parseFloat(user.winRate || 0);
      const trades = user.trades || 0;

      // Must have: win rate >= threshold AND at least 5 trades today
      if (winRate >= botState.minSuccessRate && trades >= 5) {
        ...
        console.log(
          `✅ Found: Rank #${user.rank} | ${user.address.slice(0, 6)}... ` +
          `| ${winRate}% win rate | ${trades} trades | ${user.roi}% ROI`
        );
      }
    }
    ...
    if (botState.autoDiscoveryEnabled) {
      ...
      console.log(`🎯 Auto-added ${addedCount} wallets from leaderboard`);
    }
...
server.listen(PORT, () => {
  console.log(`🤖 Polymarket Leaderboard Tracker Bot running on port ${PORT}`);
  ...
  console.log(`📈 Min success rate for discovery: ${botState.minSuccessRate}%`);
  console.log(`🏆 Tracking: Polymarket daily leaderboard`);
});
const express = require('express');
const axios = require('axios');
const http = require('http');
